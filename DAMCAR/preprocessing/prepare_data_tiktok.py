...
def prepare_data(conf):
    user_action_df = pd.read_csv(conf.USER_ACTION, sep='\t', names=[
        'userid', 'user_city', 'feedid', 'authorid', 'feed_city', 'channel', 'finish',
        'like', 'bgm_song_id', 'did', 'date_', 'videoplayseconds'])
    user_action_df = user_action_df.sort_values(by=['date_'])
    user_action_df = user_action_df.drop_duplicates(['userid', 'feedid'], keep='last')
    feed_info_df = user_action_df[conf.FEAT_FEED_LIST].drop_duplicates(['feedid'], keep='last')

    dataset = user_action_df[['userid', 'feedid', 'date_', 'finish']].drop_duplicates(['userid', 'feedid'], keep='last')
    dataset['label'] = dataset['finish']
    dataset = dataset.drop_duplicates(['userid', 'feedid'], keep='last')
    dataset = dataset.sort_values(by=['date_'])
    dataset = dataset[['userid', 'feedid', 'label']]
    ...
    pca_text_embed_df = pd.read_csv(os.path.join(conf.ROOT_PATH + 'processed_text_features.csv'))[['feedid', 'title_len']]
    feed_features = pd.merge(feed_features, pca_text_embed_df, on='feedid', how='left')
    pca_video_embed_df = pd.read_csv(os.path.join(conf.ROOT_PATH + 'pca_video_embeddings.csv'))
    feed_features = pd.merge(feed_features, pca_video_embed_df, on='feedid', how='left')
    pca_audio_embed_df = pd.read_csv(os.path.join(conf.ROOT_PATH + 'pca_audio_embeddings.csv'))
    feed_features = pd.merge(feed_features, pca_audio_embed_df, on='feedid', how='left')
