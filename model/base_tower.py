...
class BaseTower(nn.Module):
    def __init__(self, user_dnn_feature_columns, item_dnn_feature_columns, l2_reg_embedding=1e-5,
        ...
        self.user_embedding_dict = create_embedding_matrix(self.user_dnn_feature_columns, init_std, sparse=False, device=device)
        self.item_dnn_feature_columns = item_dnn_feature_columns
        self.item_embedding_dict = create_embedding_matrix(self.item_dnn_feature_columns, init_std, sparse=False, device=device)
    ...
    def fit(self, x=None, y=None, y2=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        train_tensor_data = Data.TensorDataset(torch.from_numpy(np.concatenate(x, axis=-1)), torch.from_numpy(y), torch.from_numpy(y2),)
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            with tqdm(enumerate(train_loader)) as t:
                for _, (x_train, y_train, y2_train) in t:
                    x = x_train.to(self.device).float()
                    y = y_train.to(self.device).float()
                    y2_true = y2_train.to(self.device).float()
                    y_pred = model(x).squeeze()
                    optim.zero_grad()
                    loss = loss_func(y_pred, y.squeeze(), reduction='mean')
                    consistency_loss = F.mse_loss(y_pred, y2_true.squeeze(), reduction='mean')
                    reg_loss = self.get_regularization_loss()
                    total_loss = loss + self.lambda3 * consistency_loss + reg_loss + self.aux_loss
    ...
    def input_from_feature_columns(self, X, feature_columns, embedding_dict, support_dense=True):
        ...
        varlen_sparse_embedding_list = get_varlen_pooling_list(embedding_dict, X, self.feature_index, varlen_sparse_feature_columns, self.device)
    ...
    def compile(self, optimizer, loss=None, metrics=None, lambda3=None):
