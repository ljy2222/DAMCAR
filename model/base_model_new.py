...
from preprocessing.inputs import SparseFeat, DenseFeat, VarLenSparseFeat, create_embedding_matrix, get_varlen_pooling_list, build_input_features
...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, x2=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0., validation_data=None, shuffle=True, teacher_model=None):
        ...
        train_tensor_data = Data.TensorDataset(torch.from_numpy(np.concatenate(x, axis=-1)), torch.from_numpy(y), torch.from_numpy(np.concatenate(x2, axis=-1)))
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            if epoch == initial_epoch:  # initial epoch
                dict_student = model.state_dict()
                dict_teacher = teacher_model.state_dict()
                for key in dict_teacher:
                    dict_teacher[key] = dict_student[key]
                teacher_model.load_state_dict(dict_teacher)

            with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                for _, (x_train, y_train, x_train2) in t:
                    x = x_train.to(self.device).float()
                    y = y_train.to(self.device).float()
                    x2 = x_train2.to(self.device).float()
                    src_predict, src_domains = model(x, source=True)
                    dst_predict, dst_domains = model(x2, source=False)
                    with torch.no_grad():
                        dst_predict2, _ = teacher_model(x2, source=False)
                    src_predict, src_domains = src_predict.squeeze(), src_domains.squeeze()
                    dst_predict, dst_domains = dst_predict.squeeze(), dst_predict2.squeeze()
                    dst_predict2 = dst_domains.squeeze()
                    optim.zero_grad()
                    src_label_loss = loss_func(src_predict, y.squeeze(), reduction='mean')
                    supervise_loss = loss_func(dst_predict, dst_predict2, reduction='mean')
                    src_domain_loss = loss_func(src_domains, torch.ones(len(src_domains)).to(self.device).float(), reduction='mean')
                    dst_domain_loss = loss_func(dst_domains, torch.zeros(len(dst_domains)).to(self.device).float(), reduction='mean')
                    reg_loss = self.get_regularization_loss()
                    total_loss = src_label_loss + self.lambda1 * supervise_loss + self.lambda2 * (src_domain_loss + dst_domain_loss) + reg_loss + self.aux_loss
                    ...
                    dict_teacher = teacher_model.state_dict()
                    for key in dict_teacher:
                        dict_teacher[key] = self.alpha * dict_teacher[key] + (1 - self.alpha) * dict_student[key]
                    teacher_model.load_state_dict(dict_teacher)
                    ...
                    if verbose > 0:
                        for name, metric_fun in self.metrics.items():
                            if name not in train_result:
                                train_result[name] = []
                            train_result[name].append(metric_fun(y.cpu().data.numpy(), src_predict.cpu().data.numpy().astype('float64')))
            ...
            if verbose > 0:
                ...
                for name in self.metrics:
                    eval_str += ' - ' + name + ': {0: .4f}'.format(epoch_logs[name]) + ' - ' + 'val_' + name + ': {0: .4f}'.format(epoch_logs['val_' + name])
                print(eval_str)
    ...
    def input_from_feature_columns(self, X, feature_columns, embedding_dict, support_dense=True):
        ...
        sparse_embedding_list = [embedding_dict[feat.embedding_name](X[:, self.feature_index[feat.name][0]:self.feature_index[feat.name][1]].long())
    ...
    def compile(self, optimizer, loss=None, metrics=None, alpha=0.9, lambda1=1, lambda2=0.1):
